(function ($) {
  ...
  $(document).ready(function () {
    ...
    if ($('.personal-values-list').length === 0) {
      return;
    }
    ...
    function renderCardPairs(pairs) {
      ...
      if (!pairs || !Array.isArray(pairs) || pairs.length === 0) {
          console.warn('No card pairs provided or input is invalid:', pairs);
          $('#card-pair-container').html('<p class="text-danger">No card pairs to display. Please restart the activity.</p>');
          return;
      }

      cardPairs = pairs;

      // Validate currentPairIndex
      if (typeof currentPairIndex !== 'number' || currentPairIndex < 0 || currentPairIndex >= cardPairs.length) {
          console.warn('Invalid currentPairIndex:', currentPairIndex, 'Card pairs length:', cardPairs.length);
          $('#card-pair-container').html('<p class="text-danger">Invalid pair index. Please refresh the page or restart the activity.</p>');
          return;
      }
      ...
      // Safeguard against undefined or malformed card data
      if (!pair || !pair.card1 || !pair.card2) {
          console.warn('Invalid card pair structure at index', currentPairIndex, ':', pair);
          container.html('<p class="text-danger">Oops! Something went wrong with the card data. Please refresh or restart.</p>');
          return;
      }
